...
export function AbsenceSelector({ absences, setAbsences }: Props) {
  const [name, setName] = useState("");
  const [reason, setReason] = useState("");

  const add = () => {
    if (!name.trim() || !reason) return;
    setAbsences([...absences, { name: name.trim(), reason }]);
    setName("");
    setReason("");
  };
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mt-1">
        ...
        <Button type="button" size="icon" variant="outline" onClick={add} disabled={!name.trim() || !reason}>
